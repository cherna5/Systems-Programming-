...
//unformatted text Handler
char * request_plain = "HTTP/1.0 200 OK\r\n"
        "Content-type: text/plain; charset=UTF-8\r\n\r\n";


//image handler in the form of JPEG
...
//-----------------------------------------------------------------
//404 error page type and server update 
char *error = "HTTP/1.0 404 NOT FOUND\r\n"
      "Content-type: text/html; charset=UTF-8\r\n\r\n %s";

//extra html stuff
char *err_pic = "<DOCTYPE html>\n"
  "<html>\n"
  "<body>\n"
  "<title>Error 404 Page not found...</title>"  
  "<h1>trololololoololloloollololloloooolollololoololollolool</h1>\n"  
  "<h2>NAH FAM!</h2>\n"  
  "<img src=\"https://i.imgflip.com/2a8g1r.jpg\" alt=\"Mountain View\" style=\"width:500px;height:400px;\">\n"
  "<body>"  
  "<html>\n"
  "<body>\n";

//function to pass in 404 page 
char *get_error(char *filename)
{
  char *new_file = malloc(512);
  sprintf(new_file, error, err_pic);
  return new_file;

}
//-----------------------------------------------------------------
//Step 4 
//handling requested path
...
char * directory_path(char *fileName )
{
  ...
  char * buffer = malloc (512*128);
  
  ///////
  sprintf(buffer, "HTTP/1.0 200 OK\r\n" "Content-type: text/html; charset=UTF-8\r\n\r\n");
  
  sprintf(buffer + strlen(buffer), index_hdr, fileName, fileName);

  DIR *p_dir = opendir(fileName);
  char tmp_file[512];
  struct dirent *ep_dir;

  if (p_dir != NULL)
  {
    while ((ep_dir = readdir(p_dir)) != NULL)
    {
      sprintf(tmp_file, "%s%s", tmp_file, ep_dir->d_name);

      if (is_regular_file(tmp_file))
      {
        //debugging purposes for log 
        printf("--checking-- file:  %s\n", tmp_file);
        printf("--regular file -> %s\n", ep_dir->d_name);
        sprintf(buffer + strlen(buffer), index_body, tmp_file +1, ep_dir->d_name);
      }
      else if (is_regular_directory(tmp_file))
      {
        printf("**checking-- file:  %s\n", tmp_file);
  ...
  (void) closedir(p_dir);
  sprintf(buffer + strlen(buffer),index_ftr , tmp_file+1, ep_dir->d_name);
  return buffer;
}
//-----------------------------------------------------------------
...
void serve_request(int client_fd){
  ...
  if((read_fd =open(filename,0,0))!= -1)
  {
      ...
      {
        //send(client_fd, req_str(requested_file), strlen(req_str(requested_file)), 0);
          //send(client_fd,request_string(requested_file),strlen(request_string(requested_file)),0);
          send(client_fd,get_filetype(filename),strlen(get_filetype(filename)),0);
    ...
    else 
    {
      ...
      send(client_fd, get_error(filename), strlen(get_error(filename)), 0);
    }
  close(read_fd);
  close(client_fd);
}
